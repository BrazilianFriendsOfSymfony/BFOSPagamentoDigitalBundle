...
class PagamentoManager
{
    ...
    public function retornoPagamentoDigital(Request $request){
        ...
        $this->logger->debug('METHOD:: ' . ($request->getMethod() == 'POST'?'POST':'GET'), 'notice');
        $this->logger->debug('POST:: ' . print_r($request->get('POST'), true), 'notice');
        $this->logger->debug('GET:: ' . print_r($request->get('GET'), true), 'notice');
